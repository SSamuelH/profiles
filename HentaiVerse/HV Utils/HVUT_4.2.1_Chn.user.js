...
const settings = {
    ...
    tlMap: {
        ...
        // MENU start
        // Character
        "Character": "角色面板", "Equipment": "装备", "Abilities": "技能", "Training": "训练", "Item Inventory": "物品仓库", "Settings": "设置",

        // Bazaar
        "Item Shop": "物品商店", "The Shrine": "雪花祭坛", "The Market": "交易市场", "Monster Lab": "怪物实验室", "MoogleMail": "莫古利邮局", "Weapon Lottery": "武器彩票", "Armor Lottery": "防具彩票",

        // Armory
        "Organize": "装备管理", "Modify": "装备改装", "Repair": "装备维修", "Soulbind": "装备魂绑", "Purchase": "购买装备", "Sell": "装备出售", "Salvage": "装备拆解",

        // Battle
        "The Arena": "竞技场", "The Tower": "塔楼", "Ring of Blood": "浴血擂台", "GrindFest": "压榨界", "Item World": "道具界",
        // MENU end
...
/* END OF SETTINGS */
const tlMap = settings.tlMap;
const CNFlag = settings.translateToCN;
...
function inputTranslator(p) {if(!CNFlag) return p;return tlMap[p]?tlMap[p]:p;}
...
const $config = {
    ...
    `);
        ...
        $config.data.forEach((o) => {
            ...
            if (o.input === 'textarea') {
                $input(['button', '默认'], o.node.div, null, () => { $config.set_input(o); });
            }
...
const $battle = {
    ...
    `);
        ...
        $battle.node.items = $element('ul', $battle.node.div, ['.hvut-bt-items', { dataset: { header: '物品库' } }]);
    ...
    load_repair: async function (equips) {
        ...
        if (cost) {
            ...
            let notice_text = CNFlag ?
                '已全部修复完毕' :
                'Everything is fully repaired';
            $element('li', $battle.node.repairall, notice_text);
        }
    ...
    update_link: function (eq) {
        ...
        if (eq.info.condition === 0 || eq.info.energy === 0) {
            ...
            $element('span', eq.node.link, text);
        } else if (eq.data.charms_damaged) {
            eq.node.name.classList.add('hvut-warn');
            let text = CNFlag ?
                '更换护符和囊袋' :
                'Replace Charms & Pouches';
            $element('a', eq.node.link, { textContent: text, href: `?s=Bazaar&ss=am&screen=modify&eqids=${eq.info.eid}` });
    ...
    buy_items: async function (name, count) {
        let confirm_text = CNFlag ?
            `你要购买 ${count} x ${name} 吗?` :
            `Would you like to buy ${count} x ${name}?`;
    ...
    load_items: async function () {
        ...
        items.forEach(([name, count]) => {
            ...
            console.log('load_items: ', name)
            const stock = $item.count(name);

            let name_text = CNFlag ? inputTranslator(name) : name;
            const textContent = `${name_text} (${stock})`;
...
_top.node = {};
_top.menu = {
    'Character': { s: 'Character', ss: 'ch', label: 'CH', default: 'CH', title: 'Character', CNLabel: '主页', CNTitle: '角色面板' },
    'Equipment': { s: 'Character', ss: 'eq', label: 'EQ', default: 'EQ', title: 'Equipment', CNLabel: '已穿戴', CNTitle: '装备' },
    'Abilities': { s: 'Character', ss: 'ab', label: 'AB', default: 'AB', title: 'Abilities', CNLabel: '技能', CNTitle: '技能' },
    'Training': { s: 'Character', ss: 'tr', label: 'TR', default: 'TR', title: 'Training', server: 'persistent', CNLabel: '训练', CNTitle: '训练' },
    'Item Inventory': { s: 'Character', ss: 'it', label: 'IT', default: 'IT', title: 'Item Inventory', CNLabel: '物品库', CNTitle: '物品仓库' },
    'Settings': { s: 'Character', ss: 'se', label: 'SE', default: 'SE', title: 'Settings', CNLabel: '设置', CNTitle: '设置' },

    'Item Shop': { s: 'Bazaar', ss: 'is', label: 'IS', default: 'IS', title: 'Item Shop', CNLabel: '物品店', CNTitle: '物品商店' },
    'The Shrine': { s: 'Bazaar', ss: 'ss', label: 'SS', default: 'SS', title: 'The Shrine', CNLabel: '祭坛', CNTitle: '雪花祭坛' },
    'The Market': { s: 'Bazaar', ss: 'mk', label: 'MK', default: 'MK', title: 'The Market', CNLabel: '市场', CNTitle: '交易市场' },
    'Monster Lab': { s: 'Bazaar', ss: 'ml', label: 'ML', default: 'ML', title: 'Monster Lab', server: 'persistent', CNLabel: '实验室', CNTitle: '怪物实验室' },
    'MoogleMail': { s: 'Bazaar', ss: 'mm', label: 'MM', default: 'MM', title: 'MoogleMail', CNLabel: '邮箱', CNTitle: '莫古利邮局' },
    'Weapon Lottery': { s: 'Bazaar', ss: 'lt', label: 'LT', default: 'LT', title: 'Weapon Lottery', server: 'persistent', CNLabel: '武器彩票', CNTitle: '武器彩票' },
    'Armor Lottery': { s: 'Bazaar', ss: 'la', label: 'LA', default: 'LA', title: 'Armor Lottery', server: 'persistent', CNLabel: '防具彩票', CNTitle: '防具彩票' },

    //'The Armory': { s: 'Bazaar', ss: 'am', label: 'AM', default: 'AM', title: 'The Armory' },
    'Organize': { g: 'Armory', s: 'Bazaar', ss: 'am', screen: 'organize', label: 'OR', default: 'OR', title: 'Organize', CNLabel: '装备库', CNTitle: '装备管理' },
    'Modify': { g: 'Armory', s: 'Bazaar', ss: 'am', screen: 'modify', label: 'MO', default: 'MO', title: 'Modify', CNLabel: '改装', CNTitle: '装备改装' },
    'Repair': { g: 'Armory', s: 'Bazaar', ss: 'am', screen: 'repair', label: 'RE', default: 'RE', title: 'Repair', CNLabel: '维修', CNTitle: '装备维修' },
    'Soulbind': { g: 'Armory', s: 'Bazaar', ss: 'am', screen: 'soulbind', label: 'SB', default: 'SB', title: 'Soulbind', CNLabel: '绑定', CNTitle: '装备魂绑' },
    'Purchase': { g: 'Armory', s: 'Bazaar', ss: 'am', screen: 'purchase', label: 'PU', default: 'PU', title: 'Purchase', CNLabel: '购买', CNTitle: '购买装备' },
    'Sell': { g: 'Armory', s: 'Bazaar', ss: 'am', screen: 'sell', label: 'SL', default: 'SL', title: 'Sell', CNLabel: '出售', CNTitle: '装备出售' },
    'Salvage': { g: 'Armory', s: 'Bazaar', ss: 'am', screen: 'salvage', label: 'SA', default: 'SA', title: 'Salvage', CNLabel: '拆解', CNTitle: '装备拆解' },

    'The Arena': { s: 'Battle', ss: 'ar', label: 'AR', default: 'AR', title: 'The Arena', CNLabel: '竞技场', CNTitle: '竞技场' },
    'The Tower': { s: 'Battle', ss: 'tw', label: 'TW', default: 'TW', title: 'The Tower', server: 'isekai', CNLabel: '塔楼', CNTitle: '塔楼' },
    'Ring of Blood': { s: 'Battle', ss: 'rb', label: 'RB', default: 'RB', title: 'Ring of Blood', CNLabel: '擂台', CNTitle: '浴血擂台' },
    'GrindFest': { s: 'Battle', ss: 'gr', label: 'GR', default: 'GR', title: 'GrindFest', CNLabel: '压榨界', CNTitle: '压榨界' },
    'Item World': { s: 'Battle', ss: 'iw', label: 'IW', default: 'IW', title: 'Item World', CNLabel: '道具界', CNTitle: '道具界' },
};
...
_top.init = function () {
    ...
    let link_div_classname = CNFlag ? '.hvut-top-links hvut-top-links-cn' : '.hvut-top-links';
    const links_div = $element('div', _top.node.div, [link_div_classname]);
...
const $persona = {
    ...
    check_warning: function (doc) {
        ...
        console.log("warn : ", $qsa('#stamina_restore > div > div', doc).map((d) => d.textContent.trim()))
        console.log("warn : ", $qsa('#stamina_restore > div > div', doc).map((d) => d.textContent.trim()).filter(text => text))
        console.log("warn : ", $qsa('#stamina_restore .fcr', doc).map((d) => d.textContent.trim()))
        _player.warn = $qsa('#stamina_restore > div > div', doc).map((d) => d.textContent.trim()).filter(text => text); // Repair weapon, Repair armor, Check equipment, Check attributes
        ...
        if ((_player.condition.includes('Stamina: Exhausted') || (CNFlag && _player.condition.includes('你已经筋疲力尽')))
            || _player.accuracy || _player.stamina <= $config.settings.warnLowStamina) {
            _top.node.div.classList.add('hvut-top-warn');
            _top.node.stamina.firstElementChild.classList.add('hvut-warn');
        } else if (_player.condition.includes('Stamina: Great') ||
            (CNFlag && _player.condition.includes('你现在精力充沛'))) {
...
_bottom.show_equip = async function () {
    ...
    _bottom.node.equip.textContent = `仓库容量: ${usage} / ${capacity}`;
...
if (_query.s === 'Character' && _query.ss === 'ch' || $id('persona_outer')) {
    ...
    _ch.exp = {
        ...
        open: function () {
            ...
            _ch.exp.node.level = $input(['number', null, '等级', 'before'], _ch.exp.node.div, { value: _player.level, min: 1, max: 600, style: 'width: 50px;' });
            const ass = $config.get('tr_level', {})['Assimilator'] || 0;
            _ch.exp.node.ass = $input(['number', null, '同化者等级', 'before'], _ch.exp.node.div, { value: ass, min: 0, max: 25, style: 'width: 30px;' });
            _ch.exp.calc();
        },
...
if (_query.s === 'Character' && _query.ss === 'eq') {
    ...
    _eq.init = function () {
        ...
        $input(['button', '装备一览'], _eq.node.buttons, null, () => { _eq.popup_init(); });
...
if (_query.s === 'Character' && _query.ss === 'ab') {
    ...
    _ab.init = function () {
        ...
        $input(['button', '能力点计算器'], $id('ability_outer'), ['!position: absolute; top: 20px; left: -80px; width: 90px; white-space: normal;'], () => { _ab.calc.toggle(); });
    };
...
if (_query.s === 'Bazaar' && (_query.ss === 'lt' || _query.ss === 'la')) {
    if ($config.settings.lotteryNotification && $qs('img[src$="lottery_next_d.png"]')) {
        ...
        let text = CNFlag ? "在每个页面上均展示本次头奖装备" : "Show this lottery in the bottom bar";
        $input(['checkbox', null, text], div, { checked: !_lt.json[_query.ss].hide }, { change: (e) => { _lt.toggle(e.target.checked); } });
    }
