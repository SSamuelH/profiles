...
const filename = '{user-name} (@{user-id}) - ({date-time-cn}) {short-text} ({status-id})_{file-type}'
const TMD = (function () {
    ...
    return {
        ...
        click: async function (btn, status_id, is_exist, index) {
            ...
            info['date-time-cn'] = this.formatDate(tweet.created_at, 'YYYY年MM月DD日 hhmm', true)
            info['full-text'] = tweet.full_text.split('\n').join(' ').replace(/\s*https:\/\/t\.co\/\w+/g, '').replace(/[\\/|<>*?:"\u200b-\u200d\u2060\ufeff]/g, v => invalid_chars[v])
            info['short-text'] = tweet.full_text.split('\n').join(' ').replace(/\s*https:\/\/t\.co\/\w+/g, '').replace(/[\\/|<>*?:"\u200b-\u200d\u2060\ufeff]/g, v => invalid_chars[v]).substring(0, 30)
