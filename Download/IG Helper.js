...
(function ($, Mediabunny) {
    ...
    async function createMediaListDOM(postURL, selector, message) {
        console.log("createMediaListDOM")
     ...
     * @param  {String|null}  metadata.text
     * @return {Promise}
     */
    ...
    function getSaveFileName(downloadLink, metadata) {
        let { username, sourceType, timestamp, filetype, shortcode, index, uid , text} = metadata;
        ...
        const shortName = `${username} - (${year}${month}${day}) (${shortcode}).${filetype}`;
    ...
    async function createSaveFileElement(downloadLink, object, metadata) {
        console.log("createSaveFileElement: ", metadata)
        let { username, sourceType, filetype, shortcode , text} = metadata;
